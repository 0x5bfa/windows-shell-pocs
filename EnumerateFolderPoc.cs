...
return EnumerateFolderPoc.Run(args);

internal static class EnumerateFolderPoc
{
	public static int Run(string[] args)
	{
		if (args is ["--help"] or ["-h"] or ["/?"])
		{
			Console.WriteLine("Usage: dotnet run --file EnumerateFolderPoc.cs -- [path]");
			return 0;
		}

		string path = args.Length > 0 ? args[0] : Environment.GetFolderPath(Environment.SpecialFolder.DesktopDirectory);
